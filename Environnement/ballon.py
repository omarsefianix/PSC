...
class Ballon:
    def __init__(self, air:Air, time:dict, pos:list, z:float, target:tuple, seul = False, d_min=50) -> None:
        ...
        self.s = pb.s0*pb.c                      #charge de la batterie (max c)
    ...
    def reinitialise(self,pos,z,time,target,air):
        self.pos = pos                      #position: latitude puis longitude
        self.z = z                          #altitude par la pression
        self.dzdt = 0                      #altitude au pas de temps précédent
        self.mv = pb.conversion_p_to_mv(z) - pb.m/pb.V  #masse volumique dans le ballon
    ...
    def get_reward(self, dist_inv = False) -> float:
        ...
        if delta < self.d_min:
            ...
            f = pb.cd * np.exp((self.d_min - delta)/pb.T)
        if self.de == 0:
            r = 1
        else:
            r = 0.95 - 0.3 * self.de
    ...
    def next_state(self, action:int) -> None:
        ...
        self.z = self.air.adjust_pression(self.z)
